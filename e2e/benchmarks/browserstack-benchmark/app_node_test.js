...
describe('test outfile capabilities', () => {
  const config = {
  ...
  it('tests writing capabilities', function() {
    const filePath = './benchmark_test_results.json';
    const file = require(filePath);

    write(filePath, results);
    expect(file).toEqual(results);
  });

  it('tests promise capabilities', async () => {
    const mockBenchmarkResult =
        jasmine.createSpy('mockBenchmarkResult').and.callFake(function(tabId) {
          return new Promise((resolve) => {resolve(results[tabId])});
        });

    let testResults = await benchmark(config, mockBenchmarkResult);
    let formattedResults = {};
    for (let i = 0; i < 4; i++) {
      let result = testResults[i]['value']
      formattedResults[result['tabId']] = result;
    }

    expect(mockBenchmarkResult.calls.count()).toEqual(4);
    expect(mockBenchmarkResult)
        .toHaveBeenCalledWith(
            'iPhone_XS_1' || 'Samsung_Galaxy_S20_1' || 'Windows_10_1' ||
            'OS_X_Catalina_1');
    expect(formattedResults).toEqual(results);
  });
});
