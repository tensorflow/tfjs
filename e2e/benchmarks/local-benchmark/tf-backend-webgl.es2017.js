...
`;
  ...
  class MathBackendWebGL extends tf.KernelBackend {
      ...
      readSync(dataId) {
          ...
          let result;
          if (dtype === 'complex64') {
              const realValues = this.getValuesFromTexture(complexTensorInfos.real.dataId);
              const imagValues = this.getValuesFromTexture(complexTensorInfos.imag.dataId);
  ...
  function complex(args) {
      ...
      // possible.
      const realVals = backend.readSync(real.dataId);
      const imagVals = backend.readSync(imag.dataId);
      const complexInfo = backend.makeTensorInfo(real.shape, 'complex64');
      const complex = backend.texData.get(complexInfo.dataId);
      const realDataId = backend.write(realVals, real.shape, 'float32');
      const realTensorInfo = { dataId: realDataId, shape: real.shape, dtype: 'float32' };
      const imagDataId = backend.write(imagVals, imag.shape, 'float32');
      const imagTensorInfo = { dataId: imagDataId, shape: imag.shape, dtype: 'float32' };
...
`;
   ...
   * =============================================================================
   */
  const { maxImpl: maxImplCPU, transposeImpl: transposeImplCPU, addImpl: addImplCPU, multiplyImpl: multiplyImplCPU, subImpl: subImplCPU, } = shared;
...
`;
  ...
  return result;
`;
  const atan2KernelFunc = binaryKernelFunc({ opSnippet: ATAN2, packedOpSnippet: ATAN2_PACKED });
  const atan2Config = {
      kernelName: tf.Atan2,
      backendName: 'webgl',
  ...
  function real(args) {
      ...
      const realVals = backend.readSync(resultData.complexTensorInfos.real.dataId);
      const dataId = backend.write(realVals, input.shape, 'float32');
      const tensorInfo = { dataId, shape: input.shape, dtype: 'float32' };
  ...
  function imag(args) {
      ...
      // possible
      const resultData = backend.texData.get(input.dataId);
      const imagVals = backend.readSync(resultData.complexTensorInfos.imag.dataId);
...
return a / b;`;
  ...
  return result;
`;
  const divKernelFunc = binaryKernelFunc({ opSnippet: DIV, packedOpSnippet: DIV_PACKED, checkOutOfBounds: true });
  const divConfig = {
      kernelName: tf.Div,
      backendName: 'webgl',
  ...
  class FFTProgram {
      constructor(op, inputShape, inverse) {
        console.log("INSIDE FFT PROGRAM");
          this.variableNames = ['real', 'imag'];
...
`;
  ...
  const squaredDifferenceKernelFunc = binaryKernelFunc({ opSnippet: SQUARED_DIFFERENCE, packedOpSnippet: SQUARED_DIFFERENCE });
