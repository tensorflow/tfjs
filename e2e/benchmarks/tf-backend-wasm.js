...
(function (global, factory) {
  ...
  const divConfig = createBinaryKernelConfig('Div', supportsFullBroadcast$1);

  const supportsFullBroadcast$2 = false;
  const equalConfig = createBinaryKernelConfig('Equal', supportsFullBroadcast$2, 'bool');
  ...
  const floorDivConfig = createBinaryKernelConfig('FloorDiv', supportsFullBroadcast$3);
  ...
  const greaterEqualConfig = createBinaryKernelConfig('GreaterEqual', supportsFullBroadcast$5, 'bool');

  const supportsFullBroadcast$6 = false;
  const lessConfig = createBinaryKernelConfig('Less', supportsFullBroadcast$6, 'bool');
  ...
  const lessEqualConfig = createBinaryKernelConfig('LessEqual', supportsFullBroadcast$7, 'bool');

  const logConfig = createUnaryKernelConfig('Log');

  const supportsFullBroadcast$8 = false;
  const logicalAndConfig = createBinaryKernelConfig('LogicalAnd', supportsFullBroadcast$8, 'bool');
  ...
  const maximumConfig = createBinaryKernelConfig('Maximum', supportsFullBroadcast$9);
  ...
  const minimumConfig = createBinaryKernelConfig('Minimum', supportsFullBroadcast$a);
  ...
  function setup$k(backend) {
      wasmFunc$4 = backend.wasm.cwrap('NonMaxSuppressionV5', 'number', // Result*
      [
          'number',
          'number',
          'number',
  ...
  const notEqualConfig = createBinaryKernelConfig('NotEqual', supportsFullBroadcast$c, 'bool');
  ...
  const powConfig = createBinaryKernelConfig('Pow', supportsFullBroadcast$d);
  ...
  function setup$u(backend) {
      wasmFunc$6 = backend.wasm.cwrap('Softmax', null /* void */, [
          'number',
          'number',
          'number',
          'number' // batch
  ...
  const subConfig = createBinaryKernelConfig('Sub', supportsFullBroadcast$e);
  ...
  var tfjsBackendWasmThreaded = createCommonjsModule(function (module, exports) {
  ...
  async function init() {
      ...
      return new Promise((resolve, reject) => {
          ...
          const wasm = tfjsBackendWasmThreaded(factoryConfig);
          const voidReturnType = null;
          wasm.mainScriptUrlOrBlob = new Blob([`var _scriptDir = undefined; var WasmBackendModule = ` +
                  tfjsBackendWasmThreaded.toString()], { type: 'text/javascript' });
