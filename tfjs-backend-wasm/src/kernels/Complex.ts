...
import {Complex, ComplexInputs, engine, registerKernel, TensorInfo} from '@tensorflow/tfjs-core';
...
export function complex(args: {backend: BackendWasm, inputs: ComplexInputs}):
  ...
  outData.complexTensors = {
    real: engine().keep(backend.clone(real)),
    imag: engine().keep(backend.clone(imag))
  };
