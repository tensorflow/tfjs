...
export function registerCompareKernel(
    kernelName: string, supportsBroadcast: boolean) {
  let wasmFunc:
      (aId: number, aShape: Uint8Array, aShapeLen: number, bId: number,
       bShape: Uint8Array, bShapeLen: number, inputType: number, outId: number
      ) => void;
  ...
  function kernelFunc(args: {backend: BackendWasm, inputs: CompareInputs}):
      TensorInfo {
    ...
    const out = backend.makeOutput(newShape, 'bool');
...
interface CompareInputs extends NamedTensorInfoMap {
  a: TensorInfo;
  b: TensorInfo;
}
