...
namespace {
template <typename T>
size_t clamp(const size_t min, const size_t x, const size_t max) {
  return std::max(min, std::min(x, max));
}

template <typename T>
size_t map_coord(const float out_coord, const size_t len, const size_t mode) {
  switch (mode) {
    case 1:
...
float map_coord_reflect(const float out_coord, const size_t len) {
  ...
  if (in_coord < 0) {
    if (len <= 1) {
      ...
      size_t sz2 = 2 * len;
      if (in_coord < sz2) {
        in_coord = sz2 * static_cast<size_t>(-in_coord / sz2) + in_coord;
      }
...
float map_coord_wrap(const float out_coord, const size_t len) {
  ...
  if (in_coord < 0) {
    if (len <= 1) {
      ...
      size_t sz = len - 1;
      in_coord += len * (static_cast<size_t>(-in_coord / sz) + 1);
    }
  } else if (in_coord > len - 1) {
    if (len <= 1) {
      in_coord = 0;
...
float read_with_fill_value(const float* image, const size_t image_height,
                           ...
                           cosnt size_t batch, const size_t y, const size_t x,
  ...
  // the input.
  return (0 <= y && y < image_height && 0 <= x && x < image_width) {
    return image[offset];
  }
  else {
    return fill_value;
...
float nearest_interpolation(const float* image, const size_t image_height,
  ...
  return read_with_fill_value(
      ...
      batch, static_cast<size_t>(std::round(y)),
      static_cast<size_t>(std::round(x)), channel, fill_value);
}
...
float bilinear_interpolation(const float* image, const size_t image_height,
  ...
  float value_yfloor =
      (x_ceil - x) * read_with_fill_value(image, image_height, image_width,
                                          ...
                                          batch, static_cast<size_t>(y_floor),
                                          static_cast<size_t>(x_floor), channel,
                                          fill_value) +
      (x - x_floor) * read_with_fill_value(image, image_height, image_width,
                                           batch_stride, row_stride, col_stride,
                                           batch, static_cast<size_t>(y_floor),
                                           static_cast<size_t>(x_ceil), channel,
                                           fill_value);

  float value_yceil =
      (x_ceil - x) * read_with_fill_value(image, image_height, image_width,
                                          batch_stride, row_stride, col_stride,
                                          batch, static_cast<size_t>(y_ceil),
                                          static_cast<size_t>(x_floor), channel,
                                          fill_value) +
      (x - x_floor) * read_with_fill_value(image, image_height, image_width,
                                           batch_stride, row_stride, col_stride,
...
void Transform(const int* image_id, const T* transforms_id,
               const bool is_batch_transform, const size_t batch,
               const size_t out_height, const size_t num_channels,
               const size_t image_width, const size_t image_height,
               const int32_t* strides_ptr,
               const size_t strides_size const size_t interpolation_mode_id,
               const size_t fill_mode_id, const float fill_value, T* out_id) {
  ...
  // Initialize output with fillValue.
  std::fill(out_buf, out_buf + out_size, fill_value);
    ...
    for (size_t out_y = 0; out_y < out_height; ++out_y) {
      for (size_t out_x = 0; out_x < out_width; ++out_x) {
        for (size_t channel = 0; channel < num_channels; ++channel) {
          ...
          // fill_mode_id.
          const x = map_coord(in_x, image_width, fill_mode_id);
          const y = map_coord(in_y, image_height, fill_mode_id);

          float val = 0;
          switch (interpolation_mode_id) {
            case 1:
              val = nearest_interpolation(image, image_height, image_width,
                                          batch_stride, row_stride, col_stride,
                                          b, y, x, channel, fill_value);
              break;
            case 2:
              val = bilinear_interpolation(image, image_height, image_width,
                                           batch_stride, row_stride, col_stride,
                                           b, y, x, channel, fill_value);
              break;
            default:
