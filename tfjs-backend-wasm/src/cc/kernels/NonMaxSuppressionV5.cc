...
const Result* NonMaxSuppressionV5(const size_t boxes_id, const size_t scores_id,
  ...
  auto my_comp = [](const Candidate i, const Candidate j) {
    return i.score < j.score ||
           ((i.score == j.score) && (i.box_index > j.box_index));
  };

  // Construct a max heap by candidate scores.
  std::priority_queue<Candidate, std::deque<Candidate>, decltype(my_comp)>
      candidate_priority_queue(my_comp);
  ...
  int32_t* selected_indices_data =
      static_cast<int32_t*>(malloc(selected_indices.size() * sizeof(int32_t)));
  ...
  return new Result{selected_indices_data, selected_indices.size(),
                    selected_scores_data, selected_scores.size()};
}
