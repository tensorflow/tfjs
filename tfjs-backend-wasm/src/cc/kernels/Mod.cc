  ...
  switch (dtype) {
    case DType::float32:
      binary_xnn_f32(a_id, a_shape_ptr, a_shape_len, b_id, b_shape_ptr,
                     b_shape_len, out_id, xnn_create_mod_nd_f32,
                     xnn_setup_mod_nd_f32);
      break;
    case DType::int32:
      binary_i32(a_id, b_id, out_id, mod<int32_t>);
      break;
    case DType::boolean:
      binary_bool(a_id, b_id, out_id, mod<bool>);
      break;
    default:
      util::warn("Mod for tensor ids %d and %d failed. Unknown dtype %d", a_id,
                 b_id, dtype);
  }
}
