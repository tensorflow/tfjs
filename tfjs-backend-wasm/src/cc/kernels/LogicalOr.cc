...
namespace {
inline bool logical_or(bool a, bool b) { return a || b; }
}  // namespace
...
#endif
void LogicalOr(const int a_id, const size_t *a_shape_ptr, const int a_shape_len,
  ...
  switch (input_type) {
    case DType::boolean:
      compare_bool(a_id, a_shape_ptr, a_shape_len, b_id, b_shape_ptr,
                   b_shape_len, out_id, logical_or);
      break;
    default:
      util::warn(
          "LogicalOr for tensor ids %d and %d failed. Unsupported input_type "
          "%d",
