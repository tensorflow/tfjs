...
void Dilation2DBackpropInput(int x_id, int filter_id, int dy_id, int grad_id,
  ...
  switch (dtype) {
    case DType::float32:
      Dilation2DBackpropInputImpl(x_info.f32(), filter_info.f32(),
                                  dy_info.f32(), grad_info.f32_write(), info);
      break;
    case DType::int32:
      Dilation2DBackpropInputImpl(x_info.i32(), filter_info.i32(),
                                  dy_info.i32(), grad_info.i32_write(), info);
      break;
    default:
      util::warn(
          "Dilation2DBackpropInput for tensor id %d failed. Unsupported dtype "
          "%d",
          x_id, dtype);
      break;
  }
