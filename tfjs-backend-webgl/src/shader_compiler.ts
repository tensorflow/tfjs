...
// import {getColPackedInputSamplingSnippet} from './shader_compiler_dense';
...
export function makeShader(
  ...
  inputsInfo.forEach(x => {
    ...
    if (x.shapeInfo.isUniform) {
      ...
      throw new Error('x.shapeInfo.isUniform');
    } else {
    ...
    if (program.enableShapeUniforms) {
      ...
      throw new Error('program.enableShapeUniforms');
    }
  });
...
function getColPackedOutputSamplingSnippet(
...
function getShaderPrefix(glsl: GLSL): string {
  const SHADER_PREFIX = `${glsl.version}
    ...
    ${SAMPLE_1D_CHANNEL_SNIPPET}
    ${SAMPLE_2D_CHANNEL_SNIPPET}
    ${SAMPLE_3D_CHANNEL_SNIPPET}
  `;

  return SHADER_PREFIX;
}

const SAMPLE_1D_CHANNEL_SNIPPET = `
...
const SAMPLE_2D_CHANNEL_SNIPPET = `
...
const SAMPLE_3D_CHANNEL_SNIPPET = `
...
function getColPackedSamplerFromInInfo(inInfo: InputInfo): string {
  const shape = inInfo.shapeInfo.logicalShape;
  switch (shape.length) {
    case 0:
      return getColPackedSamplerScalar(inInfo);
    case 1:
      return getColPackedSampler1D(inInfo);
    case 2:
      return getColPackedSampler2D(inInfo);
    case 3:
      return getColPackedSampler3D(inInfo);
    default:
      return getColPackedSamplerND(inInfo);
  }
}
...
export function getColPackedInputSamplingSnippet(
  ...
  let res = '';
  res += getColPackedSamplerFromInInfo(inInfo);

  const inShape = inInfo.shapeInfo.logicalShape;
  const outShape = outShapeInfo.logicalShape;
  if (inShape.length <= outShape.length) {
    res += getColPackedSamplerAtOutputCoords(inInfo, outShapeInfo);
  }
  return res;
}

function getColPackedSamplerScalar(inputInfo: InputInfo): string {
  const texName = inputInfo.name;
  const funcName = 'get' + texName.charAt(0).toUpperCase() + texName.slice(1);
  const glsl = getGlslDifferences();
  return `
...
function getColPackedSampler1D(inputInfo: InputInfo): string {
  const texName = inputInfo.name;
  const funcName = 'get' + texName.charAt(0).toUpperCase() + texName.slice(1);
  const texShape = inputInfo.shapeInfo.texShape;
  const packedTexShape = [texShape[0], Math.ceil(texShape[1] / 4)];
...
function getColPackedSampler2D(inputInfo: InputInfo): string {
  const shape = inputInfo.shapeInfo.logicalShape;
  const texName = inputInfo.name;
  const funcName = 'get' + texName.charAt(0).toUpperCase() + texName.slice(1);
  const texShape = inputInfo.shapeInfo.texShape;
...
function getColPackedSampler3D(inputInfo: InputInfo): string {
  const shape = inputInfo.shapeInfo.logicalShape;
  const texName = inputInfo.name;
  const funcName = 'get' + texName.charAt(0).toUpperCase() + texName.slice(1);
  const texShape = inputInfo.shapeInfo.texShape;
  ...
  if (shape[0] === 1) {
    ...
    const params = ['b', 'row', 'col'];
    return `
        ${getColPackedSamplerFromInInfo(newInputInfo)}
        vec4 ${funcName}(int b, int row, int col) {
          return ${funcName}(${getSqueezedParams(params, keptDims)});
        }
      `;
...
function getColPackedSamplerND(inputInfo: InputInfo): string {
  const shape = inputInfo.shapeInfo.logicalShape;
  const rank = shape.length;
  const texName = inputInfo.name;
  const funcName = 'get' + texName.charAt(0).toUpperCase() + texName.slice(1);
...
function getColPackedSamplerAtOutputCoords(
    inputInfo: InputInfo, outShapeInfo: ShapeInfo) {
  const texName = inputInfo.name;
  const texFuncSnippet = texName.charAt(0).toUpperCase() + texName.slice(1);
  const funcName = 'get' + texFuncSnippet + 'AtOutCoords';
