...
function getKernelConfigBody(
  ...
  const kernelFuncInputsType =
      inputs.length > 0 ? `inputs: ${kernelName}Inputs` : '';
  const kernelFuncBackendType = 'backend: MathBackendWebGL';
  const kernelFuncAttrsType =
      attrs.length > 0 ? `attrs: ${kernelName}Attrs` : '';
  const kernelFuncArgsTypeParts = [
    kernelFuncInputsType, kernelFuncBackendType, kernelFuncAttrsType
  ].filter(p => p.length > 0);
...
async function run(kernelFuncNames: string[]) {
  const backendFilePath = 'src/backend_webgl.ts';
  ...
  const backendClass = backendFile.getClass('MathBackendWebGL');
