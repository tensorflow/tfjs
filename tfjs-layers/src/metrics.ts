...
function truePositives(yTrue: Tensor, yPred: Tensor): Tensor {
  return tidy(() => {
    return tfc.logicalAnd(
        tfc.equal(yTrue, 1), tfc.cast(tfc.sum(tfc.equal(yPred, 1)), 'float32'));
  });
}
...
function falseNegatives(yTrue: Tensor, yPred: Tensor): Tensor {
  return tidy(() => {
    return tfc.logicalAnd(
        tfc.equal(yTrue, 1), tfc.cast(tfc.sum(tfc.equal(yPred, 0)), 'float32'));
  });
}
...
function falsePositives(yTrue: Tensor, yPred: Tensor): Tensor {
  return tidy(() => {
    return tfc.logicalAnd(
        tfc.equal(yTrue, 0), tfc.cast(tfc.sum(tfc.equal(yPred, 1)), 'float32'));
  });
}
