 ...
 */
 ...
 describeMathCPU('SigmoidRange: Symbolic', () => {
   it('Correct output shape with min = 1 and max = 5', () => {
     const layer = tfl.layers.sigmoidRange({min: 1, max: 5});
     const x = new tfl.SymbolicTensor('float32', [2, 3, 4], null, null, null);
     const y = layer.apply(x) as tfl.SymbolicTensor;
     expect(y.shape).toEqual(x.shape);
   });
 ...
 describeMathCPUAndGPU('SigmoidRange: Tensor', () => {
   it('min = 1 and max = 5', () => {
     const layer = tfl.layers.sigmoidRange({min: 1, max: 5});
     const x = tensor2d([[-1, -2], [0, 3]], [2, 2]);
     const y = layer.apply(x) as Tensor;
     expectTensorsClose(
          // y, tensor2d([[1, 3], [5, 4.8103]], [2, 2]));
          y, tensor2d([[99, 0], [5, 0]], [2, 2]));
   });
 });
