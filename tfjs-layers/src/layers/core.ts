...
class MultiHeadAttentionLayer extends Layer
{
  ...
  private feedForwardKernel: LayerVariable = null;
  private feedForwardBias: LayerVariable = null;
  ...
  constructor(args: TransformerLayerArgs)
  {
    ...
    this.feedForwardKernel = null;
    this.feedForwardBias = null;
  }
  ...
  public build(inputShape: Shape | Shape[]): void
  {
    ...
    if (this.feedForwardKernel == null)
    {
      this.feedForwardKernel = this.addWeight(
        "feedForwardKernel",
        [inputLastDim, this.valueDim],
        null,
        this.kernelInitializer,
      ...
      this.feedForwardBias = this.addWeight(
        "feedForwardBias",
        [this.valueDim],
        null,
        this.biasInitializer,
  ...
  ): { feature: Tensor, attention: Tensor }
  {
    return tidy(() =>
    {
      ...
      const flatScaledInput = K
        .dot(flatInput, this.inputKernel!.read())
        .add(this.inputBias!.read());

      // [B*toks, depth] => [B, toks, depth]
      const scaledInput = flatScaledInput.reshape([batchSize, this.maxTokens, -1,]);
      ...
      const flatQuery = K
        .dot(flatScaledInput, this.queryKernel!.read())
        .add(this.queryBias!.read());

      // [B*toks, emb] => [B*toks, emb]
      const flatKey = K
        .dot(flatScaledInput, this.keyKernel!.read())
        .add(this.keyBias!.read());

      // [B*toks, emb] => [B*toks, emb]
      const flatValue = K
        .dot(flatScaledInput, this.valueKernel!.read())
        .add(this.valueBias!.read());

      // [B*toks, emb] => [B, toks, depth]
      const query = flatQuery.reshape([batchSize, this.maxTokens, -1]);

      // [B*toks, emb] => [B, toks, depth]
      const key = flatKey.reshape([batchSize, this.maxTokens, -1]);

      // [B*toks, emb] => [B, toks, depth]
      const value = flatValue.reshape([batchSize, this.maxTokens, -1]);

      // [B, toks, emb] => [B, nHeads, toks, depth//nHeads]
      const queryT = transpose(
        query.reshape([batchSize, -1, this.numHeads, this.depth / this.numHeads,]),
        [0, 2, 1, 3]
      );
      ...
      const valueT = transpose(
        value.reshape([batchSize, -1, this.numHeads, this.depth / this.numHeads,]),
        [0, 2, 1, 3]
      );
      ...
      // TODO Need to bring matMul from tfjs-core or find a solution for this op
      // [B, nHeads, toks, depth//nHeads] => [B, nHeads, toks, toks]
      const matmuQK = matMul(queryT, keyT, false, true);

      // TODO need to bring sqrt and cast from tfjs-core or find a solution for this op
      // [B, nHeads, toks, toks] => [B, nHeads, toks, toks]
      let logits = matmuQK.div(sqrt(cast(this.depth, "float32")));
      ...
      // TODO bring scalar from tfjs-core or find a solution for this op
      // [B, nHeads, toks, toks] => [B, nHeads, toks, toks] : with -inf where mask was one
      logits = logits.add(scalar(1.0).sub(toBroadcastMask).mul(-1e9));

      // TODO bring softmax from tfjs-core or find a solution for this op
      // [B, nHeads, toks, toks] => [B, nHeads, toks, toks]
      const attentionWeights = softmax(logits, -1);
      const matmulAttentionValue = matMul(attentionWeights, valueT, true, false);

      ///////////////////////////////
      // Reshape & Final attention //
      ///////////////////////////////
      // [B, nHeads, toks, toks] => [B, nHead*toks, depth]
      const matmulAttentionValueT = transpose(matmulAttentionValue, [0, 2, 1, 3]);
      const concatAttention = matmulAttentionValueT.reshape(
        [batchSize, -1, this.depth]);


      // [B, nHead*toks, depth] => [B * toks, depth]
      const flattenConcatAttention = concatAttention.reshape(
        [batchSize * this.maxTokens, -1]);

      // [B * toks, depth] => [B * toks, depth]
      const flattenAttention = K
        .dot(flattenConcatAttention, this.multiHeadKernel!.read())
        .add(this.multiHeadBias!.read());


      // [B * toks, depth] => [B, toks, depth]
      const attention = flattenAttention.reshape([batchSize, this.maxTokens, -1]);
      ...
      // [Batch * toks, depth] => [Batch * toks, depth]
      const flatFf1 = K
        .dot(flattenNormalizedLatent, this.feedForwardKernel1!.read())
        .add(this.feedForwardBias1!.read());

      // TODO bring leaky relu from tfjs-core or find a solution for this op
      // [Batch * toks, depth] => [Batch * toks, depth]
      const flatReLUFf1 = leakyReLU().apply(flatFf1);

      // [Batch * toks, depth] => [Batch * toks, depth]
      const flatFf2 = K
        .dot(flatReLUFf1, this.feedForwardKernel2!.read())
        .add(this.feedForwardBias2!.read());

      // TODO bring dropout from tfjs-core or find a solution for this op
      // [Batch * toks, depth] => [Batch * toks, depth]
      const flatDroppedFf2 = dropout({ rate: this.dropout }).apply(flatFf2);

      // [Batch * toks, depth] => [Batch, toks, depth]
      const outFf2 = flatDroppedFf2.reshape([batchSize, this.maxTokens, -1]);
      ...
      if (this.pool) { return { feature: output.mean(1), attention } }
      else { return { feature: output, attention } }
    });
  }
