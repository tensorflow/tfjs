...
describe('decode images', () => {
  it('decode png', async () => {
    ...
    const uint8array =
        await getUint8ArrayFromImage('test_objects/image_png_test.png');
  ...
  it('decode png 4 channels', async () => {
    ...
    const uint8array = await getUint8ArrayFromImage(
        'test_objects/image_png_4_channel_test.png');
  ...
  it('decode bmp', async () => {
    const beforeNumTensors: number = memory().numTensors;
    const uint8array =
        await getUint8ArrayFromImage('test_objects/image_bmp_test.bmp');
  ...
  it('decode jpg', async () => {
    const beforeNumTensors: number = memory().numTensors;
    const uint8array =
        await getUint8ArrayFromImage('test_objects/image_jpeg_test.jpeg');
  ...
  it('decode gif', async () => {
    const beforeNumTensors: number = memory().numTensors;
    const uint8array =
        await getUint8ArrayFromImage('test_objects/gif_test.gif');
    const imageTensor = await tf.node.decodeImage(uint8array);
    expect(imageTensor.dtype).toBe('int32');
