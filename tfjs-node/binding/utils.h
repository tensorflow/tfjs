...
#define ENSURE_NAPI_OK(env, status)                   \
  if (!EnsureNapiOK(env, status, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_TF_OK(env, status)                   \
  if (!EnsureTFOK(env, status, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_CONSTRUCTOR_CALL(env, info)                   \
  if (!EnsureConstructorCall(env, info, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_VALUE_IS_OBJECT(env, value)                  \
  if (!EnsureValueIsObject(env, value, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_VALUE_IS_STRING(env, value)                  \
  if (!EnsureValueIsString(env, value, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_VALUE_IS_NUMBER(env, value)                  \
  if (!EnsureValueIsNumber(env, value, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_VALUE_IS_ARRAY(env, value)                  \
  if (!EnsureValueIsArray(env, value, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_VALUE_IS_TYPED_ARRAY(env, value)                 \
  if (!EnsureValueIsTypedArray(env, value, __FILE__, __LINE__)) \
    return;
...
#define ENSURE_VALUE_IS_NOT_NULL(env, value)                 \
  if (!EnsureValueIsNotNull(env, value, __FILE__, __LINE__)) \
    return;
