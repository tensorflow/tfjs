...
parser.addArgument(
    '--source', {help: 'Path to source typescript files', required: false});

parser.addArgument(
    '--dest', {help: 'Path to json output files', required: false});
...
const destDir = destPath || 'tfjs-converter/python/tensorflowjs/op_list/';
if (!fs.existsSync(destDir)) {
  fs.mkdirSync(destDir);
}

// Go over all .ts files in src/operations/op_list and convert them to json.
const srcDir = sourcePath || 'tfjs-converter/src/operations/op_list';
