...
describeWithFlags('fromPixelsAsync', BROWSER_ENVS, () => {
  ...
  it('fromPixelsAsync, reshape, then do tf.add()', async () => {
    ...
    pixels.data[2] = 4;
    pixels.data[3] = 255;  // Not used.
    const a = (await tf.browser.fromPixelsAsync(pixels, 3))
                  .reshape([1, 1, 1, 3]);
  ...
  it('fromPixelsAsync for PixelData type', async () => {
    ...
    const a = (await tf.browser.fromPixelsAsync(pixelsA, 3)).toFloat();
    const b = (await tf.browser.fromPixelsAsync(pixelsB, 3)).toFloat();
    const res = a.add(b);
    expect(res.shape).toEqual([1, 1, 3]);
    expect(res.dtype).toBe('float32');
    expectArraysClose(await res.data(), [260, 9, 11]);
