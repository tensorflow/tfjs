...
function runRaggedGather(
  ...
  return {
    rtNestedSplits: output.rtNestedSplits.dataSync(),
    rtDenseValues: output.rtDenseValues.dataSync(),
...
describeWithFlags('raggedRange ', ALL_ENVS, () => {
  ...
  it('RangeSizeOverflow', async () => {
    expect(
        ...
        .toThrowError('Requires ((limit - start) / delta) <= 2147483647');
  });
  ...
  it('InvalidArgsStarts', async () => {
    expect(
        () => runRaggedGather(
            ...
            tf.tensor1d([2, 1, 1, -1], 'int32')))
        .toThrowError('starts must be a scalar or vector');
  });
  ...
  it('InvalidArgsLimits', async () => {
    expect(
        () => runRaggedGather(
            ...
            tf.tensor1d([2, 1, 1, -1], 'int32')))
        .toThrowError('limits must be a scalar or vector');
  });
  ...
  it('InvalidArgsDeltas', async () => {
    expect(
        ...
        .toThrowError('deltas must be a scalar or vector');
  });

  it('InvalidArgsShapeMismatch', async () => {
    expect(
        () => runRaggedGather(
            tf.tensor1d([0, 5, 8, 5], 'int32'), tf.tensor1d([7, 8, 1], 'int32'),
            tf.tensor1d([2, 1, 1, -1], 'int32')))
        .toThrowError('starts, limits, and deltas must have the same shape');
  });
  ...
  it('does not have memory leak.', async () => {
    ...
    const result = runRaggedGather(starts, limits, deltas);
