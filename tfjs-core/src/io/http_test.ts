...
describeWithFlags('http-load fetch', NODE_ENVS, () => {
  ...
  it('1 group, 2 weights, 1 path', async () => {
    ...
    expect(new Float32Array(new CompositeArrayBuffer(modelArtifacts.weightData)
      .slice())).toEqual(floatData);
  });
...
describeWithFlags('http-load', BROWSER_ENVS, () => {
  describe('JSON model', () => {
    ...
    it('1 group, 2 weights, 1 path', async () => {
      ...
      expect(new Float32Array(new CompositeArrayBuffer(
        modelArtifacts.weightData).slice())).toEqual(floatData);
    ...
    it('1 group, 2 weight, 2 paths', async () => {
      ...
      expect(modelArtifacts.modelTopology).toEqual(modelTopology1);
      expect(modelArtifacts.weightSpecs).toEqual(weightManifest1[0].weights);
      expect(new Float32Array(new CompositeArrayBuffer(modelArtifacts
        .weightData).slice())).toEqual(new Float32Array([1, 3, 3, 7, 4]));
    });
    ...
    it('2 groups, 2 weight, 2 paths, Int32 and Uint8 Data', async () => {
      ...
      expect(modelArtifacts.weightSpecs)
          .toEqual(
              weightsManifest[0].weights.concat(weightsManifest[1].weights));
      expect(new Int32Array(new CompositeArrayBuffer(modelArtifacts.weightData)
    ...
    it('weights only', async () => {
      ...
      expect(modelArtifacts.weightSpecs)
          .toEqual(
              weightsManifest[0].weights.concat(weightsManifest[1].weights));
      expect(new Int32Array(new CompositeArrayBuffer(
        modelArtifacts.weightData).slice(0, 12)))
          .toEqual(new Int32Array([1, 3, 3]));
      expect(new Float32Array(new CompositeArrayBuffer(
        modelArtifacts.weightData).slice(12, 20)))
