...
import {KernelProfile} from './profiler';
...
type KernelInfo = {
  name: string;
  bytesAdded?: number; totalBytesSnapshot: number;
  tensorsAdded?: number; totalTensorsSnapshot: number; inputShapes: number[][];
  outputShapes: number[][];
};
...
export class Engine implements TensorTracker, DataMover {
  ...
  runKernelFunc<T extends Tensor|Tensor[], I extends NamedTensorMap>(
    ...
    let kernelProfiles: KernelProfile[];
    this.scopedRun(
