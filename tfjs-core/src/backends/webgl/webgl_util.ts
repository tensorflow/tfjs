...
export function canBeRepresented(num: number): boolean {
  if (environment().getBool('WEBGL_RENDER_FLOAT32_ENABLED') || num === 0 ||
...
export function validateTextureSize(width: number, height: number) {
  const maxTextureSize = environment().getNumber('WEBGL_MAX_TEXTURE_SIZE');
  if ((width <= 0) || (height <= 0)) {
...
export function getTextureShapeFromLogicalShape(
  ...
  let maxTexSize = environment().getNumber('WEBGL_MAX_TEXTURE_SIZE');
  if (isPacked) {
