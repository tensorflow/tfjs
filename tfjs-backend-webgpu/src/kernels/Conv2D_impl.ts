...
import {backend_util, env, TensorInfo, Tensor, Tensor3D, Tensor4D} from '@tensorflow/tfjs-core';
...
export function conv2dWithIm2Col({
  ...
  const im2Col = backend.compileAndRun(im2ColProgram, [xSqueezed]);
  const im2Col3D =
  ...
  const matMulProgram = new MatMulPackedProgram(
      ...
      transposeB);
  const result: Tensor = backend.compileAndRun(matMulProgram, [im2Col3D, w2Row]);
  if (isChannelsLast) {
