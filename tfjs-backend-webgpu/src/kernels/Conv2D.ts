...
import {backend_util, Conv2D, Conv2DAttrs, Conv2DInputs, env, engine, KernelConfig, KernelFunc} from '@tensorflow/tfjs-core';
...
export function conv2d(
    args:
        {inputs: Conv2DInputs, attrs: Conv2DAttrs, backend: WebGPUBackend}) {
  ...
  if (convInfo.filterHeight === 1 && convInfo.filterWidth === 1 &&
    ...
    const output =
        engine().makeTensorFromDataId(dataId, convInfo.outShape, x.dtype, backend);
    ...
    return backend.compileAndRun(program, [x, filter], output, dimensions);
}
