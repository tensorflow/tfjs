...
export function unaryKernelFunc(
  ...
  return ({inputs, backend}) => {
    ...
    if (webgpuBackend.shouldExecuteOnCPU([x as Tensor]) && cpuKernelImpl != null) {
...
export function binaryKernelFunc({
  ...
  return ({inputs, backend}) => {
    ...
    if (webgpuBackend.shouldExecuteOnCPU([a as Tensor, b as Tensor]) && cpuKernelImpl != null) {
      ...
      const output =
          engine().makeTensorFromDataId(dataId, outShape, $dtype, webgpuBackend);
      return output;
    }
    const program = getBinaryProgram(opSnippet, a.shape, b.shape);
    if (boolType) {
      const dataId = webgpuBackend.write(null /*values*/, program.outputShape, 'bool');
    ...
    const dataId = webgpuBackend.write(null /*values*/, program.outputShape, $dtype);
    const output =
        engine().makeTensorFromDataId(dataId, program.outputShape, $dtype, webgpuBackend);
