...
export class WebGPUBackend extends KernelBackend {
  ...
  conv2d(x: Tensor4D, filter: Tensor4D, convInfo: backend_util.Conv2DInfo):
    ...
    if (workPerThread === -1) {
      ...
      const key = 'Conv2DNaive|' + convInfo.outShape.join(',') + '|' +
          convInfo.dataFormat;
      program = this.getAndSaveProgram(key, () => {
        return new Conv2DNaiveProgram(convInfo);
      });
    } else {
      const key = 'Conv2DMM|' + convInfo.outShape.join(',') + '|' +
          convInfo.dataFormat + '|' + convInfo.filterHeight + '|' +
          convInfo.filterWidth + '|' + convInfo.inChannels + '|' +
          convInfo.inShape + '|' + workPerThread;
      program = this.getAndSaveProgram(key, () => {
