...
export class WebGPUBackend extends KernelBackend {
  ...
  private shaderSource: {[key: string]: string} = {};
  ...
  public runWebGPUProgram(
    ...
    if (this.isGetShader) {
      if (shaderKey in this.shaderSource) {
        return output;
      }
      const outputData = {dtype: output.dtype, shape: output.shape};
      this.shaderSource[shaderKey] =
          webgpu_program.makeShader(inputsData, outputData, program);
      return output;
    }
  ...
  /** Returns shader generated by func */
  // tslint:disable-next-line: no-any
  getShader(shaderSpecs: any, func: (shaderSpecs: any) => void):
      {[key: string]: string} {
    env().set('WEBGPU_CPU_FORWARD', false);
    this.isGetShader = true;
    func = func ? func : makeOp;
    if (Array.isArray(shaderSpecs)) {
      shaderSpecs.forEach(shaderSpec => {
        func(shaderSpec);
      });
    } else {
      func(shaderSpecs);
...
// tslint:disable-next-line: no-any
function makeOp(shaderSpec: any) {
  const {name, inputs, args} = shaderSpec;
  ...
  try {
    if (typeof args === 'undefined') {
      tfOpFunc(...tensorArray);
    } else {
      tfOpFunc(...tensorArray, ...args);
    }
  } catch {
