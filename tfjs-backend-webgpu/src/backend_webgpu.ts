...
export type WebGPUKernelTime = {
  name: string; query: number[];
};
...
function allTimeFunction(
  ...
  const queryResults: WebGPUKernelTime[] = [];
...
export class WebGPUBackend extends KernelBackend {
  ...
  private async getBufferData(info: TensorBufferInfo):
    ...
    if (this.supportTimeQuery && env().getBool('DEBUG_QUERY')) {
      await this.getAllTimeFromQuerySet(this.querySet);
    }
  ...
  async getQueryTime(query: GPUQuerySet, start: number): Promise<number> {
    if (this.supportTimeQuery) {
      return this.getTimeFromQuerySet(query, start);
    } else {
      return 0;
    }
  }
  ...
  public runWebGPUProgram(
    ...
    const debugQueryFlag = env().getBool('DEBUG_QUERY');
    ...
    if (shouldTimeProgram) {
      this.activeTimers.push({
        name: programName,
        query: this.getQueryTime(this.querySet, this.querySetIndex - 2)
      });
    }
    return output;
  }
  ...
  async getTimeFromQuerySet(querySet: GPUQuerySet, start: number) {
    const timeElapsed = await this.getTimeFromQuerySetCommon(
        this.querySet, 2, start, [], timeFunction);
    return timeElapsed;
  }

  async getAllTimeFromQuerySet(querySet: GPUQuerySet) {
    if (this.querySetIndex !== 0) {
      const lastQuerySetIndex = this.querySetIndex;
      this.querySetIndex = 0;
      const lastKernelNames = this.kernelNames;
      this.kernelNames = [];
      await this.getTimeFromQuerySetCommon(
          this.querySet, lastQuerySetIndex, 0, lastKernelNames,
          allTimeFunction);
    }
  }

  async getTimeFromQuerySetCommon(
      querySet: GPUQuerySet, querySetIndex: number, start: number,
      kernelNames: string[],
      f: (arrayBuf: BigUint64Array, querySetSize: number,
          kernelNames: string[]) => number) {
    ...
    this.currentCommandEncoder.resolveQuerySet(
        querySet, start, querySetIndex, queryBuffer, 0);
