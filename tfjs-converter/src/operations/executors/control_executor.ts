...
// import {fromTensor, reserve, scatter, split} from
// '../../executor/tensor_list'; import {InternalOpAsyncExecutor, Node} from
// '../types';
...
//       const args = getParamValue('args', node, tensorMap, context) as
//       Tensor[]; const condValue = await cond.data(); if (condValue[0]) {
...
//       const data = getParamValue('tensor', node, tensorMap, context) as
//       Tensor; context.enterFrame(frameId); return [cloneTensor(data)];
//     }
//     case 'Exit': {
//       const data = getParamValue('tensor', node, tensorMap, context) as
//       Tensor; context.exitFrame(); return [cloneTensor(data)];
//     }
//     case 'NextIteration': {
//       const data = getParamValue('tensor', node, tensorMap, context) as
//       Tensor; context.nextIteration(); return [cloneTensor(data)];
//     }
//     case 'TensorArrayV3': {
...
//       const elementShape =
//           getParamValue('elementShape', node, tensorMap, context) as
//           number[];
//       const dynamicSize =
//           getParamValue('dynamicSize', node, tensorMap, context) as boolean;
//       const clearAfterRead =
//           getParamValue('clearAfterRead', node, tensorMap, context) as
//           boolean;
...
//           name, dtype, size, elementShape, identicalElementShapes,
//           dynamicSize, clearAfterRead);
...
//       const index = getParamValue('index', node, tensorMap, context) as
//       number; const writeTensor =
...
//       const numElements =
//           getParamValue(numElementsParam, node, tensorMap, context) as
//           number;
//       const maxNumElements = node.op === 'TensorListReserve' ? -1 :
//       numElements; const tensorList =
