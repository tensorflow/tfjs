...
def run():
  """Run the converter wizard.

  Returns:
    A tuple of three items:
      - dryrun: A boolean indicating whether this is a dryrun, i.e., the
        converter will not actually be invoked, but instead only the converter
        command line arguments will be printed.
      - arguments: A list of strings representing the arguments to the converter
      - output_path: A string reprensenting the path of the folder or files
  ...
  dryrun_config = [{
      'type': 'list',
      'name': 'dryrun',
      'message': ('Is this a dryrun (i.e., only prints command line for '
                  'tensorflowjs_converter?'),
      'choices': ['No', 'Yes']
  }]
  dryrun_param = PyInquirer.prompt(dryrun_config)
  dryrun = dryrun_param['dryrun'] == 'Yes'
  ...
  return dryrun, arguments, params[common.OUTPUT_PATH]


def print_output_files(output_path):
