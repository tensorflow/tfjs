...
def _convert_v3_group(group, actual_layer_name):
  ...
  if 'vars' in list_of_folder:
    ...
    for key in list_of_folder:
      group_out += _convert_v3_group(group[key])
  return group_out
...
def _check_v3_version(file):
  """Check version compatiility.

  Args:
    file_path: An metadata.json file.

  Raises:
    ValueError: if the KerasVersion of the v3 file is unsupported.
  """
  if not 'keras_version' in file:
    raise ValueError('No keras version find. Check the metadata file.')

  keras_version = file['keras_version']
  if keras_version.split('.')[0] != '2':
    raise ValueError(
        'Expected Keras version 2; got Keras version %s' % keras_version)


def _initialize_v3_output_dictionary(h5file, metafile):
  """Prepopulate required fields for all data foramts.

  Args:
    h5file: Valid h5file object.
    metafile: Valid metadata.json object.
...
def h5_v3_merged_saved_model_to_tfjs_format(h5file, meta_file, config_file,split_by_layer=False):
  ...
  The HDF5 weights file is one generated by Keras' save_model method or model.save()
...
def h5_v3_weights_to_tfjs_format(h5file, metafile, split_by_layer=False):
  """Load weight values from a Keras HDF5 file and to a binary format.
  ...
  Args:
    h5file: An instance of h5py.File, or the path to an h5py file.
    metafile: A path of metadata.json file.
    split_by_layer: (Optional) whether the weights of different layers are
      to be stored in separate weight groups (Default: `False`).
  ...
  h5file = _ensure_h5file(h5file)
  try:
    _check_v3_version(metafile)
  except ValueError:
    print("""failed to lookup keras version from the meta file.""")
